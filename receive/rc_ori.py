...
def getXboxKeys():
    ...
    while running:
        ...
        for axis_id in range(num_axes):
            # keys.append(joystick.get_axis(axis_id))
...
def dataRecv():
    ...
    while True:
        ...
        while len(data_buffer) < data_length + 4:
            data_receive = ser.read(data_length + 4 - len(data_buffer))
            data_buffer += list(data_receive)

        checksum = sum(data_buffer[3:-1]) & 0xFF
        if checksum == data_buffer[-1]:
            valid_data = data_buffer[3:-1]
            mode_list = ['ALT_HOLD', 'STABILIZE', 'LAND', 'POSHOLD', 'RTL', 'Loiter', "AutoTUNE", "GUIDED", ...]
            ...
            # attitude_pitch = valid_data[40]
            # attitude_yaw = valid_data[41]
            # attitude_roll = valid_data[42]

            # print(
            #     'fly_mode:{},fly_time：{},armed_state:{},is_armable:{},dianya:{},dianliu:{},dianliang:{},vx:{},vy:{},'
            #     'vz:{},lat:{},lon:{},alt:{},homelat:{},homelon:{},homealt:{},state:{},attitude_pitch:{},attitude_yaw:{},'
            #     'attitude_roll:{}'
            #     .format(mode_list[valid_data[0]], feixingshijian / 1000, armed_state, is_armable, dianya / 100,
            #             dianliu / 100, dianliang / 100, vx / 100, vy / 100, vz / 100, lat / 1e6, lon / 1e6, alt,
            #             homelat / 1e6, homelon / 1e6, homealt, state, attitude_pitch, attitude_yaw, attitude_roll))
            ...
            # udp_soc.sendto(send_data.encode("utf-8"), ('192.168.1.205', 56050))
            # udp_soc.sendto(send_data.encode("utf-8"), ('0.0.0.0', 12345))
            try:
                # udp_soc.sendto(send_data.encode("utf-8"), ('192.168.1.205', 56050))
                udp_soc.sendto(send_data.encode("utf-8"), ('127.0.0.1', 12345))
            except Exception as e:
                print(e)
            # print(f"Received JSON data: {json.dumps(decoded_data, indent=4)}")
        else:
            #udp_soc.close()
